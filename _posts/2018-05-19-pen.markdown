...
##### 随即角度，在[0,360)内随机取得角度，然后在[0,r]范围内随机取值，然后使用`sin`和`cos`来求解x和y。
java代码如下：
```
...
>明显看到中心处的密集程度高于边缘地带，事实上当角度固定时，r在[0,R)范围内随即取值。当数量更大时，坐标点是均匀分布的。<br>当r越小时，所占用的面积越小，就会显得粒子很密集。

#### 随即角度，在[0,360)内随机取得角度，取[0,1]内的随即平方根再和R相乘，然后使用`sin`和`cos`来求解x和y。
java代码如下：
```
int degree = mRandom.nextInt(360);
...
### 总结
以上就是本次`Demo`的思路、以及一些算法的解析。数学之美，令人沉醉*（数学学渣留下了悔恨的泪水。。。）*<br>**数学才是本体啊**
